 ...
 * ADDITIONS, FIXES: Douglas Oberle, Woodson HS, Fairfax VA
 */
...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener, Drawable
{
...
*/
   ...
   */
    ...
    * @return state change timestamp
    */
   public long moveColorTo(Color targetColor, int numSteps)
   {
      Color temp = moveColorTowards(this.penColor, targetColor, numSteps);
      return penColor(temp);
   }
  ...
  /**
   ...
   private static void drawBackground(Graphics2D g)
   {
      ...
      else if(backgroundMode==BACKGROUND_MODE_TILE_RELATIVE)
      {
         ...
         for(double x=((int)(left/w)-1)*w;x<=right;x+=w) 
            for(double y=((int)(bottom/h))*h;y<=top+h;y+=h) offscreen.drawImage(backgroundImage, (int)((x-centerX)*scale+width/2),(int)((y-centerY)*(-scale)+height/2), (int)Math.ceil(w*scale), (int)Math.ceil(h*scale), null);
      }
   }
   ...
   /**
    * Looks to see if the turtle is out of bounds in the current JFrame.
    * @param smidge  smidge is not null
    * @return true if smidge is out of bounds in the current JFrame, false otherwise
    */
   public static boolean outOfBounds(Turtle smidge)
   {
      double distFromCenter = smidge.distanceTo(0, 0);
      return (distFromCenter >= window.getHeight()/2 || distFromCenter >= window.getWidth()/2);
   }
