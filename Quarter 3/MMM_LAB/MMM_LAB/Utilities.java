...
//pre:  x1 - x2 != 0
//post: returns true if the point (x,y) is within (cellSize/2) units from the line segment defined by (x1,y1) & (x2, y2)
      ...
      for (int i = 0; i < code.length; i++)
      {
         for (int j = 0; j < code.length; j++)
         {
            if (code[i].compareTo(code[j]) == 0) {
               decrypted.append((char) (j + 'a')).append(" ");
               break;
            }
         }
      }
   ...
   public static String encode(String word)
   {
      ...
      for (int i = 0; i < word.length(); i++)
      {
         for (int j = 0; j < letter.length; j++)
         {
            ...
            {
               encrypted.append(code[j]).append(" ");
               break;
            }
         }
      }
         ...
         {  
            ...
            //destroy any bridge that is close to where we land  
               ...
               {//if we land on a bridge, destroy it (change it to water)
                  board[r][c][panel] = "~~~";
                  if(structures[r][c][panel]!=null)	//there might be sludge there to remove
...
//post:  removes any destroyed sludge from the array of structures
   public static void clearSludge()
   {
      for(int r=0;r<structures.length;r++)
      {
         for(int c=0;c<structures[0].length;c++)
         {
            for(int p=0; p<structures[0][0].length; p++)
            {
               ...
               if(curr!= null && curr.getName().contains("sludge") && curr.getHealth()==0)
...
//post:  automatically stomps on any structrure with a small enough health when moved over it
   public static void walkDamage(Monster curr, int r, int c, int p)
   {
      ...
      {   
         ...
         if(str!=null && str.isDestroyable() && str.getHealth() < strHealth && !str.getName().contains("sludge") && !str.getName().equals("hole")) 
         {
            if(str.getHealth() != 0)
            {
...
//			any player 2 monsters are removed if they die
   ...
   //if player1 has a twin and dies, kill the twin as well
      if(hasSplit &&  players[PLAYER1].getHealth() <= 0 && index == SPLIT_TWIN)
      {
         currDies = true;
      }
      if ((index==PLAYER2 || index == AIPLAYER || index == SPLIT_TWIN) && curr.getHealth() <= 0)
         currDies = true;
      else
      ...
      if(index==AIPLAYER || index==SPLIT_TWIN || (gameMode == CITY_SAVER && index>=AIMONSTER1 && index<=AIMONSTER4) || gameMode==EARTH_INVADERS)
