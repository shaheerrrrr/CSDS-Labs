...
//post:returns true if curr is a habitable building - used to keep track of zone-clearing bonus
   public static boolean habitable(Structure curr)
   {
      if(curr==null)
         return false;
      if(curr.getName().startsWith("TREE") || curr.getName().startsWith("ELEC") || curr.getName().contains("sludge") || curr.getName().startsWith("FUEL") || curr.getName().startsWith("hole"))
         return false;
      if(curr.getHealth()==0)
         return false;
      return true;
...
//post:returns true if board[r][c][p] is some kind of travelable street
   public static boolean isStreet(int r, int c, int p)
   {
      ...
      {
         if(str!=null)
         {//since the structure is on a street, it is either destroyed sludgr or an "overlap" structure
...
//post:returns true if board[r][c][p] is passable by a crowd
   public static boolean passableByCrowd(int r, int c, int p)
   {
      ...
      if(str!=null)
      {
         if(str.getName().startsWith("hole"))
            return false;
         if(str.getName().contains("sludge") && str.getHealth() == 0)
            return true;
         return false;
      } 
