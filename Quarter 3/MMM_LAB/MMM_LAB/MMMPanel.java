...
// when sludge is freed up by means of fire or explosion, restore spawn points.	
// read in options file for adjustable mouse-wheel speed, etc.
// small jump in move-player-smoothly when traversing from one cell to the next (easiest seen in jet fighters).
public class MMMPanel extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener
{	
   protected static boolean soundOn;                  //true if sound is on, false if no audio device found
   ...
   protected static boolean hasSplit;						//has a creature with the split ability divided into two beings?
...
//players and player array indexes (indicies?)
   ...
   protected static final int SPLIT_TWIN = 3;		//AI controlled twin for any creature with the split ability
...
//structures
   ...
   protected static String[][][] sludgeImages;	   //images for toxic-trail sludge
...
//post:  checks for a physical collision between monster curr and vehicle unit at index i
   ...
   { 
      ...
      {                                         //our monster curr is close enough to victim to trigger a collision
         ...
         {                                      //stomping on a crowd of people (civilians or squad of soldiers)
            Sound.splat();  
         ...
         else
         {
            ...
            {                                   //the train engine is destroyed, so destroy the rest of the train
               ...
               {                                //the train car is destroyed, so destroy the caboose
                  trainInPanel[panel] = false;
                  for(int tp=TRAIN1; tp<=TRAIN3; tp++)
                  {
                     if(players[tp] instanceof Vehicle)
                     {
                        ...
                        if(trainPart.getName().equals("TRAIN caboose"))
                        {
                           ...
                           explosions.add(new Explosion("SMALL", trainPart.getX()-(cellSize/2), trainPart.getY()-(cellSize/2), explosionImages, animation_delay));
                           players[tp]=new Player();
                        }//give removed train cars a temporary name so we can remove them at the end of the method without disrupting the index i of the greater loop that traverses through players
                     }
                  }	 
               }
            if(victim.isFlying())
            {                                   //aircraft (planes, helicopters)
               ...
               {                                //all other ground units (motorcycles, cars, busses, jeeps, tanks, artillery)
         ...
         for(int j=0; j<projectiles.size(); j++)			//see if projectiles hit any enemies
         {
            ...
            //mind controlled vehicles can shoot other vehicles - so see if the shooter is mind controlled   
               ...
               else								//***MONSTER GETS HIT
               {
                  ...
                  {
                     if (!shot.getType().equals("BULLET"))
                     {
                        dam *= 2;
                        curr.damage(dam);
                     }
                  }
                  else if (gameMode!=EARTH_INVADERS && curr.isFragile())	//WoeMantis takes more damage
...
//post:  If a creature has split and we are close enough (within half of cellSize), recombine them and give curr a random number of health points back between 50 and 100  
//       If the creature hasn't split and we have 100 health, split it creating a new creature at curr's location and remove a random number of health points from curr between 25 and 75
//       uses players[SPLIT_TWIN] for holding the twin that splits away from curr
   public static void splitTwin(Monster curr)
   {
      if (gameMode == EARTH_INVADERS)
      {
         if (curr.getHealth() == 100 && !hasSplit)
         {
            players[SPLIT_TWIN] = new Blop(curr.getRow(), curr.getCol(), playerImages[4]);
            curr.setHealth(curr.getHealth() - (int) (Math.random() * 51 + 25));
            hasSplit = true;
         }
         else if (hasSplit)
         {
            Point og = new Point(curr.findX(cellSize), curr.findY(cellSize));
            Point twin = new Point(players[SPLIT_TWIN].findX(cellSize), players[SPLIT_TWIN].findY(cellSize));
            if (og.distance(twin) <= 0.5 * cellSize)
            {
               players[SPLIT_TWIN] = new Player();
            ...
            if(type.equals("SHRIEK"))
            {
               ...
               if(type.equals("BEAM"))
               {
                  ...
                  if(type.equals("FIRE"))
                  {
                     ...
                     if(type.equals("SLUDGE"))
                     {
                        Sound.shootWeb();
                        temp = new Projectile("sludge"+hd, curr.getX(), curr.getY(), 15, sludgeImages, SPEED, type, SPEED*8, airShot, owner, -1, -1);
                     }
                     else
 ...
 //post: even though players are moving one array cell at a time, make all players move smoothly by pixel to the next cell
   public static void movePlayerToAdjacentCell()
   {
      for(int i=0; i < players.length; i++)
      {
         ...
         {
            if (((Monster)(curr)).slimeTrail() && (!board[r][c][panel].contains("~") || board[r][c][panel] != null))
            {
               structures[r][c][panel] = new Structure("sludge", r,c,panel, sludgeImages, 15, true,true,1,-1, cellSize, (int)(Math.random()*4), (long)(Math.random()*3000));
            }
         ...
         {	//put out any fire close to the firetruck, and spray away any sludge
            for(int nr=r-1; nr<=r+1; nr++)
               for(int nc=c-1; nc<=c+1; nc++)
               {
                  ...
                  if(structures[nr][nc][panel]!=null)
                  {
                     ...
                     if (structures[nr][nc][panel].getName().contains("sludge") && Math.random() < .025)
         ...
         {	//up command for player 1 || player 2
            ...
            //While underground or flying, a monster should be able to traverse to a location above or below her even if there is a structure there.
            {
               ...
               //can't move up in panels 0, 1 or 2
                  ...
                  else //if(!((p3toggle && playerIndex==2) || (hasSplit && playerIndex==3) || (gameMode==CITY_SAVER && monsterIndex.contains(playerIndex))))
            ...
            {					//move up
               if(k==KeyEvent.VK_W)	  //we can't walk from under an impassable structure to behind it (unless we destroy it first)
               {
                  if((curr2.getBodyDirection() == UP && r2>1 && MapBuilder.noStructure(r2-1, c2, panel)))
                  {
                     ...
                     if(str != null && str.getName().contains("sludge") && str.getHealth() != 0 && !curr2.getName().endsWith("tank")  && !curr2.getName().endsWith("flame"))
                     {//if we are not a tank or flame tank and there is sludge above us, don't move into it (just change orientation)
                  ...
                  if(k==KeyEvent.VK_S)		//move down
                  {
                     if((curr2.getBodyDirection() == DOWN && r2 < board.length-2 && MapBuilder.noStructure(r2, c2, panel)))
                     {
                        Structure str = structures[r2+1][c2][panel];
                        if(str != null && str.getName().contains("sludge") && str.getHealth() != 0 && !curr2.getName().endsWith("tank")  && !curr2.getName().endsWith("flame"))
                        {//if we are not a tank or flame tank and there is sludge below us, don't move into it (just change orientation)
                     ...
                     {								//so we dont have to check to see if there is a structure here
                        if((curr2.getBodyDirection() == LEFT && c2>1))// || curr2.head360())
                        {
                           ...
                           if(str != null && str.getName().contains("sludge") && str.getHealth() != 0 && !curr2.getName().endsWith("tank")  && !curr2.getName().endsWith("flame"))
                           {//if we are not a tank or flame tank and there is sludge left of us, don't move into it (just change orientation)
                        ...
                        if(k==KeyEvent.VK_D)			//move right
                        {
                           if((curr2.getBodyDirection() == RIGHT && c2<board[0].length-2))// || curr2.head360())
                           {
                              Structure str = structures[r2][c2+1][panel];
                              if(str != null && str.getName().contains("sludge") && str.getHealth() != 0 && !curr2.getName().endsWith("tank")  && !curr2.getName().endsWith("flame"))
                              {//if we are not a tank or flame tank and there is sludge RIGHT of us, don't move into it (just change orientation)
