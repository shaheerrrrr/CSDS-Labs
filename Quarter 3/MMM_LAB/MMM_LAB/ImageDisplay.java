...
//post: fills arrays of images and creates image icons for game elements
   ...
   //sludge
      ...
      for(int i=0; i<4; i++)
         for(int frame=0; frame<4; frame++)
         {
            if(frame == 3)	//the same picture is used for the last frame of each of the 4 sludge types
               sludgeImages[i][frame][0] = "images/monsters/blop/blopGoo03.GIF";
            else
               sludgeImages[i][frame][0] = "images/monsters/blop/blopGoo" + i + frame + ".GIF";
         }
...
//post:  shows different pictures on the screen in grid format depending on the values stored in the array board
   public static void showBoard(Graphics g)	
   {
      ...
      for(int r=0;r<board.length;r++)
      {
         ...
         //draw any sludge or holes on/in the ground
            if(structures[r][c][panel]!=null && (structures[r][c][panel].getName().contains("sludge") || structures[r][c][panel].getName().startsWith("hole")))
            {
         ...
         //draw any passable or destroyed structures so that it is in front of the player - don't show sludgr or holes in the ground here because we don't want it to appear in front of any players
            if(structures[r][c][panel]!=null && !structures[r][c][panel].getName().contains("sludge") && !structures[r][c][panel].getName().startsWith("hole") && (structures[r][c][panel].getHealth()<=0 || structures[r][c][panel].isPassable()))
            {
               Structure str = structures[r][c][panel];
         ...
         //if you can't find one, show the structure even if it is passable or destroyed
            ...
            for(int i=0; i<players.length; i++)
            {
               ...
               {	//put out any fire close to the firetruck, and spray away any sludge
                  ...
                  {
                     if (structures[r][c][panel].onFire() || structures[r][c][panel].getName().contains("sludge"))
                     {
            ...
            if(!closePlayerFound && (curr!=null && curr.getHealth() <= 0) && !curr.getName().contains("sludge") && !curr.getName().startsWith("hole"))
               showStr=true;
            if(showStr)
            {	
   ...
   //so, if the player is jumping, redraw the player, then any structures below them on that city block
      for(int i=0; i<players.length; i++)
      {
         ...
         for(int row=r; row<board.length; row++)
         {
            ...
            if(str != null && !str.getName().contains("sludge") && !str.getName().startsWith("hole"))
            {
               x = c * cellSize;
               y = row * cellSize;
   ...
   public static void displayStats(Graphics g, int x, int y, int width, int height)	
   {
      ...
      else if(health > 0 || !gameStarted)
      {
         ...
         if(!toggleMap || gameMode==CITY_SAVER || gameMode==EARTH_INVADERS)
         {
            ...
            {
               if(hasSplit && players[SPLIT_TWIN].getName().equals("The-Blop"))
