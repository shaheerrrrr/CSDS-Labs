...
//			grid is to remain unchanged
   public static int [][] invert(int [][] grid)
   {
      ...
      return newarr;	//temporary statement to keep things compiling
   }
...
//			grid is to remain unchanged
   public static int [][] mirrorFlip(int [][] grid)
   {
      ...
      for (int r = 0; r < grid.length; r++)
      {
         for (int c = 0; c < grid[0].length; c++)
         {
            newarr[r][c] = grid[r][grid[0].length-1-c];
         }
      }
      return newarr;	//temporary statement to keep things compiling
   }
...
//      the data field hint will have the correct value for the number of unused hints when the method is called
   private static boolean checkForWin()
   {
      ...
      for (int r = 0; r < board.length; r++)
      {
         for (int c = 0; c < board[0].length; c++)
         {
            if ((board[r][c] == 1) && (guess[r][c] == 0 || guess[r][c] == 2 || guess[r][c] == 3))
               return false;
            if ((board[r][c]) == 0 && (guess[r][c] == 1 || guess[r][c] == -1))
               return false;
         }
      }
      return true;
