...
public class SmokePuff
{
   ...
   public static final Color pacmanColor = Color.yellow;
   public static final Color pacGhostColor = Color.blue.brighter().brighter();
   ...
   public void advance()
   {
      ...
      if(opacity > 0)
         opacity--;
      if((color.equals(pacmanColor) || color.equals(pacGhostColor)) && opacity > 0)
         opacity--;
      ...
      else if(color.equals(Color.black) || color.equals(dustCloud) || color.equals(pacmanColor) || color.equals(pacGhostColor))    //smoke
      {   
         if(radius < CultimaPanel.SIZE * 4)
            radius++;   
         y -= Utilities.rollDie(0,3);
         if(y < 0)
            y = 0;
         if(CultimaPanel.windDir == LocationBuilder.EAST)
            x += Utilities.rollDie(0,3);
         else if(CultimaPanel.windDir == LocationBuilder.WEST)
            x -= Utilities.rollDie(0,3);
         else
            x += Utilities.rollDie(-2,2);
         if(x < 0)
            x = 0;
