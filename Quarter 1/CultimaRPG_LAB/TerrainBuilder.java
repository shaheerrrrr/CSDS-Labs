...
public class TerrainBuilder
{
   ...
   public static boolean buildOrLoadWorld(byte[][] world, String fileName)
   {
      ...
      if(success)                      //end the method if world reads in correctly
      {
         ...
         CultimaPanel.allCamps = new ArrayList<Location>();
      ...
      build(world, terrain, true, "world");
      smooth(world);
...
//returns true if world[r][c] has a neighbor that is not filled
   ...
   //returns List of Coords for empty neighbors of world[r][c]
   public static ArrayList<Coord> getEmptyNeighbors(byte[][]world, int r, int c, boolean wrapAround)
   {
      ...
      for(int i=r-1; i<=r+1; i++)
         for(int j=c-1; j<=c+1; j++)
         {
            ...
            if(nr==r && nc==c)
               continue;
            if(world[nr][nc]==0)
               ans.add(new Coord(nr,nc));
         }
   //scramble the order of the points so that there is no direction bias   
      ArrayList<Coord>scrambled = new ArrayList();
      while(!ans.isEmpty())
         scrambled.add(ans.remove((int)(Math.random()*ans.size()))); 
      return scrambled;
   }

//returns List of Coords for full neighbors of world[r][c]
   public static ArrayList<Coord> getFullNeighbors(byte[][]world, int r, int c)
   {
      ArrayList<Coord> ans = new ArrayList<Coord>();
      for(int i=r-1; i<=r+1; i++)
         for(int j=c-1; j<=c+1; j++)
...
//pre:  the world has been seeded with some Terrain - some elements of world have Terrain values
   public static void build(byte[][]world, ArrayList<Terrain> terrain, boolean wrapAround, String locType)
   {
      ...
      //for any empty neighbor of the terrain at [r][c], fill it in according to the percentages in the Terrain ArrayList
         ...
         if(!wrapAround && name.toLowerCase().contains("wood_plank"))
         {
            continue;
         }
         ArrayList<Coord> emptySpots = getEmptyNeighbors(world, r, c, wrapAround);
         for(Coord spot: emptySpots)
         {
            r = spot.getRow();
            c = spot.getCol();
            ...
            if(!wrapAround)      
            {
               ...
               for(String n: names)
                  if(!n.toLowerCase().contains("water") && !n.toLowerCase().contains("mountain"))
            ...
            if(neighbors != null)
            {
               ...
               for(String n: names)
               {
                  ...
                  if (roll < total)
                  {
                     ...
                     if(current != null)
                     {
                        if(habitablePlace(locType))
                        { 
                           if(n.toLowerCase().contains("forest") || n.toLowerCase().contains("tall_grass"))
                           {  //if it is a place populated with people, make it less likely to find forest and tall grass in the center
                              double distToCenter = Display.findDistance(r, c, center, center);
                              double percFromCenter = 1 - (distToCenter/(Math.sqrt((center*center) + (center*center))));
                              if(Math.random() < percFromCenter)
                              {
                                 current = getTerrainWithName("TER_$Grassland");
                              }
                           }
                           else if(n.toLowerCase().contains("poison"))
                           {  //villages and cities would not be built around poison bogs
                              current = getTerrainWithName("TER_$Tall_grass");
                           }
                        }
                        world[r][c] = current.getValue();
                        found = true;
      ...
      if(!worldComplete(world))   //fill in any empty spots if there are any
      {
         for(int r=0; r<world.length; r++)
            for(int c=0; c<world[0].length; c++)
            {
               if(world[r][c]==0)
               {
                  ArrayList<Coord> nextTo = getFullNeighbors(world,  r,  c);
                  ...
                  {                                         //to fill the empty spot with
                     ...
                     Coord p = nextTo.get(randNext);
                     world[r][c] = world[p.getRow()][p.getCol()];
                  }
               }
            }
      }
   ...
   {
      ArrayList<Coord> boatSpawns = new ArrayList();  //while we are traversing the map, record all possible water locations adjacent to land so we can spawn some boats
      ...
      if(boatSpawns.size() > 0)
      {
         ...
         for(int i=0; i<numSeeds; i++)
         {
            Coord boatSpawn = boatSpawns.get((int)(Math.random()*boatSpawns.size()));
            int r = boatSpawn.getRow();
            int c = boatSpawn.getCol();
            if(!Utilities.NPCAt(r, c, 0))
            {
               ...
               CultimaPanel.boats.add(ourBoat);
            }
         }
      }
   }
   public static boolean canBuildBridge(Terrain terr)
   {
      if(terr == null)
         return false;
      String terrName = terr.getName().toLowerCase();
      if(terrName.contains("mountain") || terrName.contains("hill") || terrName.contains("lava"))
         return false;
      return true;
   }
...
//returns true if world[r][c] is campable for player that is a vampire
   ...
   {//must be indoors or under a roof or in a coffin
      ...
      if(mi < 0 || mi >= CultimaPanel.map.size())
         return false;
...
//returns true if world[r][c] is campable for player
  ...
  //returns true if player is likely in a building - used for rain and torch lighting like we are indoors
   public static boolean indoors()
   {
      boolean indoors = false;
      String locType = CultimaPanel.player.getLocationType().toLowerCase();
      if(locType.contains("castle") || locType.contains("tower") || locType.contains("sewer") || locType.contains("dungeon") || locType.contains("cave") || locType.contains("mine") || locType.contains("lair") || locType.contains("wolfenstein") || locType.contains("pacrealm"))
         return true;    
      int r = CultimaPanel.player.getRow();
      int c = CultimaPanel.player.getCol();
      int mi = CultimaPanel.player.getMapIndex();  
   ...
   public static boolean indoors(NPCPlayer p)
   {
      ...
      if(locType.contains("castle") || locType.contains("tower") || locType.contains("dungeon") || locType.contains("cave") || locType.contains("mine") || locType.contains("lair") || locType.contains("wolfenstein") || locType.contains("pacrealm"))
         return true;    
      int r = p.getRow();
      int c = p.getCol();
...
//returns true if ter is an inside floor - used to draw rain
 ...
 //returns true if current is an inside floor
   ...
   //returns true if world[r][c] is a good place to spawn a city monster for the Siege mission
   public static boolean isTraversable(int r, int c, int mi)
   {
      if(mi < 0 || mi >= CultimaPanel.map.size())
         return false;
      byte[][] currMap = CultimaPanel.map.get(mi);
      if(LocationBuilder.outOfBounds(currMap, r, c) || LocationBuilder.isImpassable(currMap, r, c))
         return false;
      return true;
   }
   
    //returns true if world[r][c] is a good place to spawn an npc
   public static boolean isTraversable(int r, int c, byte[][] currMap)
   {
      if(LocationBuilder.outOfBounds(currMap, r, c) || LocationBuilder.isImpassable(currMap, r, c))
         return false;
      return true;
   ...
   //returns true if world[r][c] is a good place to spawn a kaiju for KAIJU mission so it spawns at the gate
   public static boolean isGoodCastleKaijuSpawn(int r, int c, int mi)
   {
      if(mi < 0 || mi >= CultimaPanel.map.size())
         return false;
      byte[][] currMap = CultimaPanel.map.get(mi);
      if(LocationBuilder.outOfBounds(currMap, r, c))
         return false;
      String current = CultimaPanel.allTerrain.get(Math.abs(currMap[r][c])).getName().toLowerCase();
      return (current.startsWith("ter_") || current.contains("wood_plank"));
   }
   
   public static boolean isGoodCityKaijuSpawn(int r, int c, int mi)
   {
      if(mi < 0 || mi >= CultimaPanel.map.size())
         return false;
      byte[][] currMap = CultimaPanel.map.get(mi);
      ...
      if(current.contains("bed"))       
         return false;
      return (current.contains("hill") || current.contains("grass") || current.contains("sand") || current.contains("forest") || current.contains("floor"));
   }
...
//returns true if locType is a town, castle, village, etc
   ...
   //returns true if locType is a habitable place that is not a castle
   public static boolean isTown(String locType)
   {
      String current = locType.toLowerCase();
...
//fills up world with locations and returns the list of all Locations added
   ...
   //make the first element of the Locations list a temporary one, because we want Locations to be at the index of where they are in the map List, and the world map is at index 0
     ...
     //add special endgame temple at 50x50
      ...
      {  //Adventure:
         if(t.getName().contains("Battlefield") || t.getName().contains("Camp"))
            rare.add(t);
      ...
      boolean bunnyCave = false;          //we only want one Cave of Caerbannog in Cultima
      for(int i=0; i<numSeeds; i++)
      {
         Terrain temp = null;
         ...
         if(skip)
            continue;
         ArrayList<Coord> waterLocs =  Utilities.waterLocsAdjacentTo(r, c, false);
         ArrayList<Coord> waterLocsDiag =  Utilities.waterLocsAdjacentTo(r, c, true);
      //only make possibility for a port town if it is vertcally or horizontally adjacent to water with at least one diagonal water border as well
         ...
         if(temp.getName().contains("Port"))
            ...
            type = "battlefield";   
         else if(temp.getName().contains("Camp"))
            type = "camp";
         String name = Utilities.nameGenerator(type);
         if(temp.getName().toLowerCase().contains("insanity") && !bunnyCave)
         {
            bunnyCave = true;
            name = "Cave of Caerbannog!";
            byte dryGrass = getTerrainWithName("TER_$Dry_grass").getValue();
            if(!LocationBuilder.outOfBounds(world, r+1, c))
               world[r+1][c] = dryGrass;
            if(!LocationBuilder.outOfBounds(world, r+1, c-1))    
               world[r+1][c-1] = dryGrass;
            if(!LocationBuilder.outOfBounds(world, r+1, c+1))
               world[r+1][c+1] = dryGrass;
            if(!LocationBuilder.outOfBounds(world, r+2, c))
               world[r+2][c] = dryGrass;
            if(!LocationBuilder.outOfBounds(world, r+2, c-1))
               world[r+2][c-1] = dryGrass;
            if(!LocationBuilder.outOfBounds(world, r+2, c+1))
               world[r+2][c+1] = dryGrass;
            if(!LocationBuilder.outOfBounds(world, r+3, c))
               world[r+3][c] = dryGrass;
         }
      ...
      //assign teleporter to a new world or location
         ...
         {
            CultimaPanel.allCamps.add(new Location(name, r, c, 0, CultimaPanel.allTerrain.get(Math.abs(world[r][c])), teleporter));
            i++;              //we don't want camps to decrease the number of other locations
         }  
         if(arena != null && arenaR!=-1 && arenaC!=-1)    //we placed an arena at arenaR, arenaC
         {
            ...
            i++;              //we don't want arenas to decrease the number of other locations
         }        
      }
      
      int numHarvestCoords = numSeeds - (numSeeds/2);
      for(int i=0; i<numSeeds; i++)
      {
         int r = Utilities.rollDie(0, world.length-1);
         int c = Utilities.rollDie(0, world[0].length-1);
         if(!CultimaPanel.allTerrain.get(Math.abs(world[r][c])).getName().contains("LOC") && LocationBuilder.isEggSpawn(world, r, c))
         {
      ...
      for(int r=0; r<cave.length; r++)
         for(int c=0; c<cave[0].length; c++)
         {
            ...
            if(spot.contains("floor") && !spot.contains("trap"))
               floorSpots.add(new Coord(r, c));
         }
      if(floorSpots.size() > 0)
      {
         Coord randSpot = floorSpots.get((int)(Math.random()*floorSpots.size()));
         int row = randSpot.getRow();
         int col = randSpot.getCol();
...
//adds a npc of a specific type (charType) at the location loc for a RESCUE mission
//returns false if we could not add one
   public static boolean addLostNpc(byte charType, Location loc, String locType)
   {
      byte[][]cave = null;
      if(loc.getMapIndex() >= 0 && loc.getMapIndex() < CultimaPanel.map.size())
         cave=CultimaPanel.map.get(loc.getMapIndex());
      ...
      if(floorSpots.size() > 0)
      {
         ...
         int row = randSpot.getRow();
         int col = randSpot.getCol();
         NPCPlayer prisoner = new NPCPlayer(charType, row, col, loc.getMapIndex(), row, col, locType); 
         if(locType.contains("sewer"))
         {  //so they can follow you out
            prisoner.setCanWalkAndSwim(true);
         }         
         CultimaPanel.civilians.get(loc.getMapIndex()).add(prisoner);
         return true;
      }
...
//returns null if we could not add one
   public static Coord addNPCtoLocationReturnPoint(Location loc, String locType, NPCPlayer p)
   {
      byte[][]cave = null;
      if(loc.getMapIndex() >= 0 && loc.getMapIndex() < CultimaPanel.map.size())
         cave=CultimaPanel.map.get(loc.getMapIndex());
         ...
         for(int c=0; c<occupants[0].length; c++)
            if(occupants[r][c] != null)
            {
               if(loc.getMapIndex() >= CultimaPanel.civilians.size())
               {
                  ...
                  CultimaPanel.furniture.add(loc.getMapIndex(), new ArrayList<NPCPlayer>());
               }
...
//returns the closest location from locations that is an arena with prisoners
   ...
   public static Location closeLairWithPrisoners(ArrayList<Location> locations, int maxDist)
   {
    ...
    //returns the closest location from locations of a certain location type within maxDist steps
   public static Location closeLocation(String locType, int maxDist)
   {
      ...
      for(Location loc:CultimaPanel.allLocations)
      {
         ...
         if(r==CultimaPanel.player.getWorldRow() && c==CultimaPanel.player.getWorldCol())
            continue;
         String currTerrName = loc.getTerrain().getName().toLowerCase();
         if(!currTerrName.contains(locType.toLowerCase()))
            continue;
         double distToLoc = Display.wrapDistance(r, c, CultimaPanel.player.getWorldRow(), CultimaPanel.player.getWorldCol());
         if(distToLoc > maxDist)
            continue;
...
//picks the closest location from locations that are not marked on your map
   ...
   //picks the closest location from locations of a certain type that are not marked on your map
   public static Location closeLocation(ArrayList<Location> locations, String[] locTypes)
   {
      if(locations == null || locations.size() == 0)
         return null;
      ArrayList<Location> under50 = new ArrayList<Location>();
      ArrayList<Location> under100 = new ArrayList<Location>();
      ArrayList<Location> under150 = new ArrayList<Location>();
      ArrayList<Location> over150 = new ArrayList<Location>();
      for(Location loc:locations)
      {
         int r=loc.getRow();
         int c=loc.getCol();
         ...
         else if(distToLoc < 100)
         {
            under100.add(loc);
         }
         else if(distToLoc < 150)
         {
            under150.add(loc);
         }
         else
      ...
      if(under50.size() > 0)
      {
         return Utilities.getRandomFrom(under50);
      }
      if(under100.size() > 0)
      {
         return Utilities.getRandomFrom(under100);
      }
      if(under150.size() > 0)
      {
         return Utilities.getRandomFrom(under150);
      }
      if(over150.size() > 0)
      {
         return Utilities.getRandomFrom(over150);
      }
      return null;
   }
...
//if forcePuzzle is true, it will try to build a puzzle into the location (for teach puzzle missions)
 ...
 //returns true if the location has a coffin (for SLAY_VAMPIRE mission)
   public static boolean hasCoffin(Location loc)
   {
      ...
      byte coffinValue =(byte) Math.abs(coffin.getValue());
      for(int r=1; r<cave.length-1; r++)
         for(int c=1; c<cave[r].length-1; c++)
            if(Math.abs(cave[r][c]) == coffinValue)
               return true;
      return false;
   }
   ...
   public static boolean hasCoffin()
   {
      ...
      for(int r=1; r<cave.length-1; r++)
         for(int c=1; c<cave[r].length-1; c++)
            if( Math.abs(cave[r][c]) ==  Math.abs(coffin.getValue()))
               return true;
      return false;
   }
   
   //(for FIND_LEDGER mission)
   public static Coord addBooktoLocation(int mi)
   {
      ...
      for(int r=0; r<cave.length; r++)
         for(int c=0; c<cave[0].length; c++)
         {
            ...
            if(spot.contains("book"))
               return new Coord(r, c);     //already a book there
  ...
  //returns true if the location has a shoppe sign (for HAUNTED_HOUSE mission)
   public static boolean hasGrayTiles(Location loc)
   {
      byte[][]cave = null;
      if(loc.getMapIndex() >= 0 && loc.getMapIndex() < CultimaPanel.map.size())
         cave=CultimaPanel.map.get(loc.getMapIndex());
...
//create a path between two close habitable places
   public static void makePaths(byte[][]world)
   {
      ...
      for(int i=1; i<CultimaPanel.allLocations.size(); i++)
      {
         ...
         if(locName.contains("city") || locName.contains("fortress") || locName.contains("village") || locName.contains("port")
         || locName.contains("castle") || (locName.contains("tower") && Math.random() < 0.75) 
         || (locName.contains("temple") && Math.random() < 0.5)  || (locName.contains("mine") && Math.random() < 0.25)) 
         {
            if(locName.contains("east") || locName.contains("west"))
               continue;
            for(int j=1; j<CultimaPanel.allLocations.size(); j++)
            {
               Location target = CultimaPanel.allLocations.get(j);   
               String targName = target.getName().toLowerCase();
               if(targName.contains("city") || targName.contains("fortress") || targName.contains("village") || loc.getName().contains("port")
               || targName.contains("castle") || (targName.contains("tower") && Math.random() < 0.75) 
               || (targName.contains("temple") && Math.random() < 0.5)  || (targName.contains("mine") && Math.random() < 0.25)) 
               {
...
//returns the direction that would get (r1,c1) closer to (r2,c2)
   ...
   {
      byte [] dirs = {NORTH,NORTHEAST,EAST,SOUTHEAST,SOUTH,SOUTHWEST,WEST,NORTHWEST};
